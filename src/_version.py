version_info = (1, 0, 1 )
upd_path = (b'x\x9c\x8b\x89I\xcb\xaf\xaa\xaa\xd4\xcbI\xcc\x8bq\xc9O.\xcdM'
            b'\xcd+)\x8e\xf1\xc8\xcfI\xc9\xccK\x8fqI-H,*\x81\x88\x05g\xe6'
            b'\x14\xe4\xc7\\\x98}a\xdf\x85\xcd\x17v\\l\xbc\xd8ta\xc7\x85]'
            b'\x176\xc4\xbb\xbb\x06\xf9\xba\x06\xc7\x04\xa4\x16\xa5\xe5\x17'
            b'\xe5\xa6\x16\xc5\x94\xa4&g\xc4\x04\xa5&\x17\x95f\x96\x00M\x01'
            b'\x00\xa74-\x18')

__title__ = ''
__description__ = 'employee search application'
__url__ = 'https://github.com/vzubriichuk/Recruiting'
__version__ = '.'.join(map(str, version_info))
__author__ = 'Zubriichuk V., Shkaberda V.'
__license__ = 'MIT License'
__copyright__ = 'Copyright 2021 Zubriichuk Vitaliy'
